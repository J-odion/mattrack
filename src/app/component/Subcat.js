...
export default function Subcat() {
  ...
  return (
    <section className="px-10 py-[96px] gap-y-[64px] w-[90%] justify-center flex flex-col mx-auto items-center">
      <div className="mb-4 flex justify-between items-center w-[60%]">
        <button
          onClick={handleAllClick}
          className={`w-[100px] flex justify-center items-center text-white border bg-[#123962] h-[50px] rounded-[34px] p-[4px] ${
      ...
      {!showTable ? (
        <div id="services" className="grid lg:grid-cols-4 gap-[24px] mb-[64px]">
          {generateMenu().map((key, index) => (
            <div
              key={index}
              className="w-[200px] flex justify-center items-center border border-[#123962] h-[50px] rounded-[34px] p-[4px]"
            >
              <button
                onClick={() => handleFilterClick(key)}
                className="w-full text-[#123962] font-semibold"
              >
                {key}
              </button>
            </div>
